...
export class IntelligenceCamera implements SceneModule {
  ...
  update({ pointer, dt, motionScale }: FrameContext): void {
    ...
    this.camera.position.set(
      this.base.x + this.drift.x,
      this.base.y + this.drift.y,
      this.base.z
    );
    ...
    // than translated.
    this.camera.lookAt(
      this.lookAt.x + this.drift.x * 0.4,
      this.lookAt.y + this.drift.y * 0.4,
      0
    );
