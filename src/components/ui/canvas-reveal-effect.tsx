...
// ============================================================
// NEXUS — CANVAS REVEAL EFFECT (21st.dev sign-in background)
// WebGL dot-matrix shader rendered through @react-three/fiber.
// Kept in its own module so the page can load it with
// `dynamic(..., { ssr: false })` — the Canvas must never render
// on the server.
// ============================================================
...
const ShaderMaterial = ({
  ...
  useFrame(({ clock }) => {
    ...
    // Respect the requested frame budget instead of burning every frame.
    const minInterval = maxFps > 0 ? 1 / maxFps : 0;
    if (timestamp - lastFrameTime.current < minInterval) return;
