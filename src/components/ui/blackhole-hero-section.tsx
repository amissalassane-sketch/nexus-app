...
// ============================================================
// NEXUS UI — BLACK HOLE HERO SECTION
//
// A physically raymarched black hole (Schwarzschild null
// geodesics, Keplerian accretion disc, relativistic beaming)
// rendered in pure WebGL — no dependency beyond React.
//
// NEXUS adaptation: the engine and its defaults below are tuned
// for the Atelier Noir art direction. The disc burns lavender
// (white-hot rim → violet → deep indigo edge) instead of the
// film's amber, so the brightest thing in frame stays on-palette.
// The film look is one prop set away:
...
export function BlackHoleHeroSection({
  ...
  // Latest-props ref, read by the RAF loop. Synced in an effect (never
  // during render) so a prop change never tears a frame mid-draw.
  const latest = {
