...
// ============================================================
// NEXUS ADMIN — AUDIT LOG, LOADING BOUNDARY
// ============================================================
      ...
      <AdminPanel padded={false}>
        ...
        <div aria-hidden="true" className="flex flex-col">
          {Array.from({ length: 8 }).map((_, index) => (
            ...
            >
              <span className="h-4 w-32 animate-pulse rounded bg-admin-surface-2 motion-reduce:animate-none" />
              <span className="h-3.5 w-40 animate-pulse rounded bg-admin-surface motion-reduce:animate-none" />
              <span className="h-3.5 w-20 animate-pulse rounded bg-admin-surface motion-reduce:animate-none" />
              <span className="ml-auto h-3 w-28 animate-pulse rounded bg-admin-surface motion-reduce:animate-none" />
            </div>
          ))}
        </div>
      </AdminPanel>

      <AdminEyebrow>Reading public.admin_audit_log…</AdminEyebrow>
    </div>
  );
}
